...
import { ref } from 'vue'
import type { AdminMemberPostDto, AdminMemberVo } from '~/types'

const { dialogRef, onDialogHide, onDialogOK, onDialogCancel } = useDialogPluginComponent()
const props = defineProps<{
  userInfo: AdminMemberVo
  companyInfo: AdminMemberVo
}>()
defineEmits([
...
const adminMemberPostDto = ref<AdminMemberPostDto>({
  ...
  password: '',
  userName: props.companyInfo.userName,
  position: props.companyInfo.position,
  userType: 'company',
  phoneNumber: props.companyInfo.phoneNumber,
...
const postAdminMember = async () => {
  await customFetch('/admin/handOrder/company/member', {
    ...
    params: {
      companySeq: props.companyInfo.companySeq
    },
    onResponse: (context) => {
      if (context.response.status === 200) {
        $q.notify({
          message: $t('COMPANY.072'),
          color: 'positive',
          icon: 'check'
        })
        onDialogOK()
        ...
        $q.notify({
          message: `${$t('COMPANY.073')} ${context.response._data.message}`,
          color: 'negative',
          icon: 'close'
        })
      }
            ...
            <q-item>
              ...
              <q-item-section side>
                <q-input
                  ...
                  no-error-icon
                  hide-bottom-space
                  :rules="[val => (val?.length ?? 0) > 0  || $t('COMPANY.059')]"
                />
              </q-item-section>
            </q-item>
            <q-item>
              ...
              <q-item-section side>
                <q-input
                  ...
                  no-error-icon
                  hide-bottom-space
                  :rules="[val => (val?.length ?? 0) > 0 || $t('COMPANY.060')]"
                />
              </q-item-section>
            </q-item>
            <q-item>
              ...
              <q-item-section side>
                <q-input
                  ...
                  hide-bottom-space
                  outlined
                  :rules="[val => (val?.length ?? 0) > 0 || $t('COMPANY.061')]"
            ...
            <q-item>
              ...
              <q-item-section side style="width: 60%;">
                <q-input
                  ...
                  :label="$t('COMPANY.074')"
                  :rules="[
                    val => !val || /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])[A-Za-z0-9!@#$%^&*]{8,}$/.test(val) || $t('COMPANY.064')
                  ]"
                  autocomplete="off"
                />
              </q-item-section>
            ...
            <q-item>
              ...
              <q-item-section side>
                <q-input
                  ...
                  hide-bottom-space
                  outlined
                  :rules="[val => (val?.length ?? 0) > 0 || $t('MEMBER.018')]"
                />
              </q-item-section>
            </q-item>
          </q-list>
