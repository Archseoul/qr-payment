...
import type { LabelTemplateToTablesDto, ShopInfoVo } from '~/types'
import MultipleTableSelectModal from '~/components/device/template/MultipleTableSelectModal.vue'
...
const labelTemplateData = await useCustomFetch<LabelTemplateClass[]>('/admin/handOrder/new/labelTemplate', {
  method: 'GET',
  params: {
    startRow,
    fetchCount,
...
const initialRowsNumberData = await useCustomFetch<number>('/admin/handOrder/new/labelTemplate/rowsNumber', {
  method: 'GET',
  params: {
    filterString: filter
  }
...
const getRowsNumber = async (filter: string) => {
  return await customFetch<number>('/admin/handOrder/new/labelTemplate/rowsNumber', {
    method: 'GET',
    params: {
      filterString: filter
    },
...
const fetchLabelTemplateList = async (startRow: number, fetchCount: number, filter: string, sortBy: string, descending: boolean) => {
  return await customFetch<LabelTemplateClass[]>('/admin/handOrder/new/labelTemplate', {
    method: 'GET',
    params: {
      startRow,
      fetchCount,
...
const applyLabelTemplateToTables = async (labelTemplateSeq: number, selectedDto:LabelTemplateToTablesDto) => {
  await customFetch('/admin/handOrder/labelTemplate/applyToTables', {
    method: 'POST',
...
const openMultipleTableSelectModal = (labelTemplateSeq: number) => {
  if (!checkPermissions(['U'])) {
    return
  }
  $q.dialog({
    component: MultipleTableSelectModal,
    componentProps: {
      shopList: shopInfo.value ?? [],
      labelTemplateSeq
    }
  }).onOk(async (selectedDto: LabelTemplateToTablesDto) => {
    $q.loading.show()
    await applyLabelTemplateToTables(labelTemplateSeq, selectedDto)
    $q.loading.hide()
    await labelTemplateData.refresh()
    initialTableDataLoad()
  })
...
const deleteTemplate = () => {
  ...
  $q.dialog({
    ...
    await customFetch('/admin/handOrder/new/labelTemplate', {
      method: 'DELETE',
...
<template>
  <div class="main-content relative-position full-height column">
    <div class="content-header">
      <span class="text-h5 text-bold">{{ $t('SIDE_MENU.017') }}</span>
    ...
    <div class="content-body col">
      <q-scroll-area class="full-height q-pr-md">
        ...
        >
          ...
          <template v-slot:body-cell-applyToTable="props">
            <q-td :props="props">
              <q-btn
                color="primary"
                :label="$t('COMMON.077')"
                flat
                @click="openMultipleTableSelectModal(props.row.labelTemplateSeq)"
              />
            </q-td>
          </template>
