...
const targetShopData = await useCustomFetch('/admin/handOrder/signup/temp/shop/' + authStore.userInfo.shopInfo?.shopCode, {
  method: 'GET'
})

const targetShop = targetShopData.data.value
...
const columns = [
  ...
  { name: 'boardTitle', label: $t('COMMON.087'), field: 'boardTitle', sortable: true },
  {
    name: 'createDt',
    label: $t('COMMON.088'),
    ...
    <div class="content-body col">
      <q-scroll-area class="full-height q-pr-md">
        <q-table
          ...
          :filter="stringFilter"
          :rows="noticeList.filter((notice) => targetShop ? ![8, 11, 12, 13].includes(notice.boardSeq ?? 0) : true)"
          :columns="columns"
          row-key="boardSeq"
          :loading="loading"
          binary-state-sort
          @request="onTableDataRequest"
          @row-click="(evt, row, index) => $router.push(`/notice/${row.boardSeq}`)"
        >
